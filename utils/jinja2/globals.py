...
def render_comment_tree(context, obj):
  def tree_from_queryset(queryset, with_feedback=False, user=None):
    ...
    cur_dict = None
    for obj in queryset:
      if cur_dict and obj.level == cur_dict['comment'].level:  # pylint: disable=unsubscriptable-object
  ...
  def sort(c, sort_with_like, depth):
    ...
    if depth:
      for x in c:
        if x['children']:
          x['children'] = sort(x['children'], sort_with_like, depth - 1)
    return c
  ...
  queryset = XtdComment.objects.filter(content_type=ctype,
                                       ...
                                       is_public=True).\
    annotate(likes__count=Sum(Case(When(flags__flag=LIKEDIT_FLAG, then=1),
                                   default=0, output_field=IntegerField()))).\
    annotate(dislikes__count=Sum(Case(When(flags__flag=DISLIKEDIT_FLAG, then=1),
                                      default=0, output_field=IntegerField())))
  user = context['user']
  if user.is_authenticated:
