  ...
  def get_context_data(self, **kwargs):
    ...
    if self.privileged:
      data["judgement_status"] = {
        "pending": self.contest.submission_set.filter(status__lt=-1).count(),
        "judged": self.contest.submission_set.filter(status__gte=-1).count(),
      }
      if self.contest.submission_set.exists():
        data["judgement_status"]["last"] = self.contest.submission_set.first().create_time
