...
class ActivityParticipantList(StaffRequiredMixin, ListView):
  ...
  def get_context_data(self, **kwargs):  # pylint: disable=arguments-differ
    ...
    data["active_member_count"] = ActivityParticipant.objects.filter(activity=data["activity"],
                                                                     is_deleted=False).count()
    data["member_count"] = ActivityParticipant.objects.filter(activity=data["activity"]).count()
    data["confirm_count"] = ActivityParticipant.objects.filter(activity=data["activity"], is_deleted=False,
