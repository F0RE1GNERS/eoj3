...
class Millionaires(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if not self.request.user.is_authenticated:
            ...
            data['my_rank'] = User.objects.filter(score__gte=self.request.user.score).exclude(
                username__contains='#').count()
    ...
    def get_context_data(self, **kwargs):
        ...
        for problem in problem_set.values():
            if problem.pk in accept_list:
                ...
                problem.personal_label = -1
        data.update(children_list=children_list, problem_list=problem_list, problem_set=problem_set,
