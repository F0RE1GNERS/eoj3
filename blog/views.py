...
class BlogUpdate(UserPassesTestMixin, UpdateView):
  ...
  def test_func(self):
    return self.request.user.is_authenticated and (
      is_admin_or_root(self.request.user) or self.get_object().author == self.request.user)
...
class GetRewardsView(ListView):
  ...
  def get_queryset(self):
    ...
    if self.request.user.is_authenticated:
      qs = self.user.blog_set.get_rewards_list(is_admin_or_root(self.request.user),
                                               self.request.user).with_likes().with_likes_flag(self.request.user)
    else:
      qs = self.user.blog_set.get_rewards_list(is_admin_or_root(self.request.user)).with_likes().with_likes_flag(
        self.request.user)
    return qs
